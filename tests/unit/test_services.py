...
def test_get_recipe_with_single_recipe(memory_repository, sample_recipe):
    ...
    expected = [{
        ...
        'author': sample_recipe.author.name,
        'images': sample_recipe.images,
        'category': sample_recipe.category.name if sample_recipe.category else None,
        'time': (sample_recipe.cook_time or 0) + (sample_recipe.preparation_time or 0),
        'nutrition': sample_recipe.nutrition,
        'calories': getattr(sample_recipe.nutrition, 'calories', None),
        'protein': getattr(sample_recipe.nutrition, 'protein', None),
        'fat': getattr(sample_recipe.nutrition, 'fat', None),
        'carbohydrates': getattr(sample_recipe.nutrition, 'carbohydrates', None),
    }]

    assert result == expected

def test_get_recipe_with_multiple_recipes(memory_repository, sample_recipe, my_author, my_category):
...
def test_get_recipe_by_id_single_recipe(memory_repository, sample_recipe):
    memory_repository.add_recipe(sample_recipe)
    result = recipe_detail_services.get_recipe_by_id(memory_repository, sample_recipe.id)
    assert result is sample_recipe

def test_get_recipe_by_id_multiple_recipes(memory_repository, sample_recipe, my_author, my_category):
    r2 = Recipe(
        recipe_id=99,
        name="Pumpkin Risotto",
        author=my_author,
    ...
    assert recipe_detail_services.get_recipe_by_id(memory_repository, sample_recipe.id) is sample_recipe
